...
void displayAST(struct node *T,int indent)
{
    ...
    if (T)
	{
	    switch (T->kind)
        {
	        case EXT_DEF_LIST:  displayAST(T->ptr[0],indent);    //显示该外部定义列表中的第一个
            ...
            case EXT_DEC_LIST:  displayAST(T->ptr[0],indent);     //依次显示外部变量名，
	        ...
	        case PARAM_LIST:    displayAST(T->ptr[0],indent);     //依次显示全部参数类型和名称，
            ...
            case STM_LIST:      displayAST(T->ptr[0],indent);      //显示第一条语句
            ...
            case DEF_LIST:      displayAST(T->ptr[0],indent);    //显示该局部变量定义列表中的第一个
